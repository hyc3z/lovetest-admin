...
# 小批量
curl -X POST http://localhost:8080/api/admin/generate-codes \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
...
# 第一页
curl -X GET "http://localhost:8080/api/admin/codes?pageSize=100" \
  -H "Authorization: Bearer $TOKEN"

# 第二页（使用上一页返回的 nextSkipToken）
curl -X GET "http://localhost:8080/api/admin/codes?pageSize=100&skipToken=100" \
  -H "Authorization: Bearer $TOKEN"

# 只查询未使用的激活码
curl -X GET "http://localhost:8080/api/admin/codes?isUsed=false&pageSize=50" \
  -H "Authorization: Bearer $TOKEN"

# 只查询已使用的激活码
curl -X GET "http://localhost:8080/api/admin/codes?isUsed=true&pageSize=50" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
TOKEN="your-jwt-token"
curl -X DELETE http://localhost:8080/api/admin/codes/TEST-CODE-001 \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
TOKEN="your-jwt-token"
curl -X DELETE http://localhost:8080/api/admin/codes/expired \
  -H "Authorization: Bearer $TOKEN"
```
